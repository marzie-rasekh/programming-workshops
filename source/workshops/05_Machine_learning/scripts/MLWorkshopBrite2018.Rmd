...
metrics <- lapply(1:length(folds), function(i) {
  ...
  pred_labels <- sapply(test_pred, function(x) {
    if (x < 0.5) {
      return(levels(test$Gender)[1])
    }
    return(levels(test$Gender)[2])
  })
  pred_labels <- factor(pred_labels, levels=levels(test$Gender))
  confusion <- caret::confusionMatrix(test$Gender, pred_labels)
